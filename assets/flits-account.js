...
window.flitsApp.flitsSplitArrayIntoChunks = function(arr, len) {
  ...
  flitsAppJquery.ajax({
    ...
    error: function (res) {
      ...
      switch (res.status) {
        default:
          try {
            flitsAppJquery(".flits-code-snippet").parent().find('.flits-old-code-snippet').addClass('flits-hidden');

            var accountPageJson = JSON.parse(flitsAppJquery(".flits-code-snippet").parent().find('.flits-old-code-snippet').html())
            var accountPageSections = [];
            var sectionsChunkSize = 5;
            var sectionParentElement = flitsAppJquery(".flits-code-snippet").parent();

            flitsAppJquery.each(accountPageJson.order, function (index, section) {
              accountPageSections.push(accountPageJson.sections[section].type);
            });
            window.flitsApp.flitsSplitArrayIntoChunks(accountPageSections, sectionsChunkSize).forEach(function (accountPageSection) {
              flitsAppJquery.get('/account?sections=' + accountPageSection.join(","), function(resp){
...
window.flitsApp.getCreditCommentMsg = function (comment) {
  switch (comment) {
    ...
    case 'Customer cancelled the order first time ever':
...
window.flitsApp.addSubmitEvents = function () {
  ...
  flitsAppJquery("#flits-form-profile").submit(function (event) {
    ...
    data.push({name: "customer_hash", value: window.flitsApp.customer_hash});data.push({name: "token", value: flitsAppJquery("#flits-token").val()});
    ...
    flitsAppJquery.ajax({
      ...
      success: function (res) {
        if (res.status) {
          ...
          // AFTER PROFILE SAVE EVENT, UPDATE CONTACT US FORM FIELD
          flitsAppJquery("#flits-contact-form .flits-form-first-name").attr('value', flitsAppJquery("input.flits-hide-profile-cancel[name='first_name']").val());
          flitsAppJquery("#flits-contact-form .flits-form-last-name").attr('value', flitsAppJquery("input.flits-hide-profile-cancel[name='last_name']").val());
          flitsAppJquery("#flits-contact-form .flits-form-email").attr('value', flitsAppJquery("input.flits-hide-profile-cancel[name='email']").val());
          flitsAppJquery("#flits-contact-form .flits-form-phone").attr('value', flitsAppJquery("input.flits-hide-profile-cancel[name='phone']").val());
        } else {
    ...
    flitsAppJquery.ajax({
      ...
      success: function (res) {
        if (res.status) {
          ...
          $li.attr("data-company", adrs.company);
